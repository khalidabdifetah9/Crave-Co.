...
const FAQ = () => {
  ...
  return (
    <div className="flex px-4 sm:px-6 md:px-10 mt-12 sm:mt-20 md:mt-30 bg-[#fcd301] py-10 sm:py-16 md:py-20 flex-col text-black space-y-6 sm:space-y-8 md:space-y-10">
      <div>
        <h1 className="uppercase text-4xl sm:text-6xl font-semibold md:text-8xl tracking-tight">key question</h1>
      </div>
      <div>
        {questions.map((qus) => {
          ...
          return (
            ...
            >
              <div 
                className="flex justify-between items-center pt-6 sm:pt-8 md:pt-13 pb-4 md:pb-5 cursor-pointer gap-4"
                onClick={() => toggleAnswer(qus.id)}
              >
                <h1 className="text-xl sm:text-2xl md:text-3xl capitalize ">{qus.question}</h1>
                <span className="text-3xl sm:text-4xl md:text-5xl font-light flex-shrink-0">
                  {isOpen ? '−' : '+'}
                </span>
              </div>
              <AnimatePresence initial={false}>
                {isOpen && (
                  ...
                  >
                    <p className="max-w-200 pb-5 text-base sm:text-lg md:text-xl text-black/70 capitalize">{qus.answer}</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
